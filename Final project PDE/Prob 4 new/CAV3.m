...
%--------------------------------------
  ...
  %...setup overall divergence
      Div = [NaN*zeros(N1adv,M-M1),zeros(N1adv,M1);zeros(N2,M2)];

  %...setup overall pressure
      Press = [NaN*zeros(N1adv,M-M1),zeros(N1adv,M1);zeros(N2,M2)];
  ...
  %...Initialize the green and blue interfaces (blue is split into east and south)
      interfS = zeros(16,1);
      interfE = zeros(1,32);
      interfW = zeros(1,32); 
  ...
  %...setup stopping criteria
      ...
      index = [1 10 100 500 1000 1500 2000];
  for i=1:2000
      display(i)
    ...
    %\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
        ...
        %...... solving for u
            ...
            %...Updating the interface boundaries 
            u1Eadv  = (u1_2D(N1adv,:)+u2_2D(1,33:end))/2;
            u2Wadv(33:end) = u1Eadv;

            %...Advecting Omega_1 and Omega_2
    ...
    %\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
        ...
        %...divergence for Omega_1
              ...
              %...reshaping dimensions and updating interface boundaries
    ...
    %\\\\\\\\\\\\\\\\\\\
          ...
          %...update the interface boundaries for Pressure
    
          [P1bc,P2bc] = updBoundaryPress(P1bc,P2bc,p10,N1,M1,p20,N2,M2);
          
          Div1 = reshape(Div1,N1adv,M1);
          Div2 = reshape(Div2,N2,M2);
          
          Div(1:N1adv,33:end) = Div1;
          Div(N1adv+1:end,:)  = Div2;
    ...
    %...solving for pressure
          ...
          while 1
            ...
            p1new    = PoissonSolve(p10,Div1-P1bc,Rp1,Ap1,Pp1,x1Len,y1Len);
            [P2bc, interfW] = setBoundary2p(p1new,N1,M1,P2bc,N2,M2);
            p2new    = PoissonSolve(p20,Div2-P2bc,Rp2,Ap2,Pp2,x2Len,y2Len);
          ...
          [p10,p20] = unify(p1new,N1,M1,p2new,N2,M2);
                    
          %...update overall Pressure
          p1_2D = reshape(p1new,N1,M1);
          p2_2D = reshape(p2new,N2,M2);

          Press(1:N1adv,33:end) = p1_2D(1:N1adv,:);
          Press(N1adv+1:end,:) = p2_2D;     
    ...
    %\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
        ...
        %...correcting omega_1 velocities
          ...
          %...update interface bouundaries and reshape domain 1
          E1_Neum = (p1_2D(N1adv,:)+p1_2D(N1adv+1,:))/2;    
          p1new = reshape(Press(1:N1adv,33:end),N1adv*M1,1);
    ...
    if max (i == index) == 1
      ...
      axis image
      axis([0 xLen 0 yLen]);
      saveas(1, sprintf('sol%i.jpg',i), 'png')
      
      figure(2);
      surf(xx,yy,Div)
      title(sprintf('Divergence at time-step = %i',i))
      xlabel('x')
      ylabel('y')
      axis tight
      saveas(2, sprintf('div%i.jpg',i), 'png')
      drawnow
      
      figure(3);
      surf(xx,yy,Press)
      title(sprintf('Pressure gradient at time-step = %i',i))
      xlabel('x')
      ylabel('y')
      axis tight
      saveas(3, sprintf('press%i.jpg',i), 'png')
      drawnow
    end
  end
