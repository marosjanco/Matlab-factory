...
%--------------------------------------
  ...
  %...setup the dimension of omega_1 and omega_2 respectively

  %...Domain omega_1 for advection
  
  N1adv = 48;
  M1 = 32;
  
%   u1adv  = zeros(N1adv,M1); u1adv = u1adv(:);
  
  u1Sadv =  zeros(N1adv,1);
  u1Nadv =  ones(N1adv,1);%ones
  u1Wadv =  zeros(1,M1);
  u1Eadv =  zeros(1,M1);
    
%   v1adv = zeros(N1adv,M1); v1adv = v1adv(:);
...
%   u2adv = zeros(Nadv1,Madv1); u2adv = u2adv(:);
...
%   v2adv = zeros(Nadv1,Madv1); v2adv = v2adv(:);
  ...
  %...setup of omega_1 and omega_2 for Diffusion and Poisson 
  N1 = 64;
  M1 = 32;
  ...
  p10   = zeros(N1,M1); p10 = p10(:);
  p20   = zeros(N2,M2); p20 = p20(:);
  
  u1    = zeros(N1,M1); u1  = u1(:);
  u2    = zeros(N2,M2); u2  = u2(:);
  v1    = zeros(N1,M1); v1  = v1(:);
  v2    = zeros(N2,M2); v2  = v2(:);
  ...
  P1bc = zeros(N1,M1); P1bc = P1bc(:);
  P2bc = zeros(N2,M2); P2bc = P2bc(:);
